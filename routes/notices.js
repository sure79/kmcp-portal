...
router.post('/', async (req, res) => {
  try {
    ...
    await req.logAndNotify({ type: 'notice', action: 'create', title: `새 공지: ${title}`, message: '', actor_id: author_id || 0, actor_name: req.session?.user?.name || '', target_page: 'notices', target_id: result.lastInsertRowid });
